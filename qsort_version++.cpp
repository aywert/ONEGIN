...
typedef int (*func_compare_t)(const void* a, const void* b);
...
//Р°СЂРіСѓРјРµРЅС‚С‹ РєРѕРјР°РЅРґРЅРѕР№ СЃС‚СЂРѕРєРё СЃРјРѕС‚СЂРµС‚СЊ РІ РїСЂР°С‚Рµ

GREETING_STATUS take_data(void);
...
GREETING_STATUS take_data(void)
{
    ...
    printf("1 - if straight, 0 - if backwards\n");
    int decider = 0;
    scanf("%d", &decider);
    
    return (decider == 1)? YES : NO;
}
